...
The security scan fetches its PR-count-derived history during the first authenticated checkout, before checkout removes credentials. The guard report, changed npm-lock check, and bundled protocol check also prepare their frozen comparison commit there and consume its exact SHA locally afterward. Manual npm-lock checks and targets without changed-check support retain their full-sweep behavior. Remaining supplemental fetches use the workflow-owned bootstrap from the runner temporary directory. Release-gate ratchets retain six merge-snapshot attempts and their existing deadlines; cleanup uncertainty or cancellation stops consumers. All six preflight `ls-remote` calls use this owner with no operation deadline (`None`), preserving their output, exact-SHA checks, and successful-empty-only annotated-to-lightweight tag fallback.
